...
// Generates build/bootstrap_pl.h from src/bootstrap.pl: a byte array
// (BOOTSTRAP_PL_SRC) plus its length (BOOTSTRAP_PL_SRC_LEN). This bakes the
// engine's Prolog-defined standard library into the binary at build time --
// unlike the bundled font, it's not a file `te` needs to find on disk next
// to the executable at runtime, since it's core engine behavior rather than
// user-swappable content.
static bool generate_bootstrap_header(void) {
    const char *src = "src/bootstrap.pl";
    const char *out = "build/bootstrap_pl.h";
    ...
    nob_sb_appendf(&header, "static const char BOOTSTRAP_PL_SRC[] = {");
    ...
    nob_sb_appendf(&header, "static const size_t BOOTSTRAP_PL_SRC_LEN = sizeof(BOOTSTRAP_PL_SRC);\n");
